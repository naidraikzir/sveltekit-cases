...
<Card>
	<div class="flex flex-col lg:flex-row gap-8 justify-between">
		<form class="lg:w-96 flex-shrink-0" on:submit|preventDefault>
			...
			<Input type="number" id="count" label="Count" class="mb-2" bind:value={form.count} min="0" />

			<div class="flex items-center justify-between mt-8 mb-4">
				<div class="font-bold">Items</div>
				<button class="text-sm rounded bg-purple-900 text-white px-2 py-1" on:click={addItem}>
					Add Item
				</button>
			</div>
